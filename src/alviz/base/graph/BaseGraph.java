...
//    }
    ...
    abstract public class Node {
        ...
        public void set_Heuristic(BaseGraph.Node G){
            this.heuristic=(this.x-G.x)*(this.x-G.x) + (this.y-G.y)*(this.y-G.y);
            this.heuristic=(float) (5* (float) sqrt(this.heuristic));
            
        }
        
        public void set_G_value()
        {
            if(this.mgParent==null)
            {
                this.dist_from_source=(float)(this.mgEdge.cost);
            }
            else
            this.dist_from_source=(float) (this.mgParent.dist_from_source+this.mgEdge.cost);
        }
        
        public void set_f_value(BaseGraph.Node G)
        {
            
            this.set_Heuristic(G);
            this.set_G_value();
            this.f_value=this.heuristic+this.dist_from_source;
        }
        
        public float get_f_value()
        {
            return this.f_value;
        }
